...
esp_err_t abrir_pv(char *partition_name, char **page_namespace,
    ...
    activa_entrada(partition_name, *page_namespace, entry_key,
                   actual_entry_index, Pulsos);
...
esp_err_t leer_contador_pf(char **pname, char **pvActual,
    ...
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND)
        ...
        *ContadorPaginaFija =
            1; // Valor por defecto en lugar de no estar asignado
...
esp_err_t leer_pagina_variable(char **pname, char **pvActual,
    ...
    *ContadorEntradaActual = entradasEnPv;
    ...
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND)
        ...
        *valorEntradaActual =
            0; // Valor por defecto en lugar de no estar asignado
        // Write
        err = nvs_set_i32(my_handle, *regActualPaginaVariable,
                          *valorEntradaActual);
        // Commit
        err = nvs_commit(my_handle);
        if (err != ESP_OK)
...
esp_err_t change_to_next_partition(char **pname, uint8_t *partition_number) {
    ...
    if (*partition_number <= 3) {
        ...
        // Llenando particion
        esp_err_t err = nvs_open_from_partition(*pname, "storage",
                                                NVS_READWRITE, &my_handle);
        if (err != ESP_OK)
            ESP_LOGE("NVS", "ERROR IN NVS_OPEN");
        //		free(*pname);
...
esp_err_t contar_pulsos_nvs(char **pname, uint8_t *partition_number,
    ...
    if (*CuentaPulsos <= PPE_LIMIT) {
        ...
        (*EntradaActual)++; // Límite de pulsos por entrada superado, cambiando
                            // a siguiente entrada
        if (*EntradaActual <= EPP_LIMIT) {
            *CuentaPulsos = 1;
            activa_entrada(*pname, namespace, &key, EntradaActual,
                           CuentaPulsos);

        } else {
            (*ContadorPvActual)++; // Límite de entradas por pagina superado se
                                   // debe cambiar de página.
            if (*ContadorPvActual <= PPP_LIMIT) {
                *EntradaActual = 1;
                *CuentaPulsos  = 1;
                abrir_pv(*pname, &namespace, &key, ContadorPvActual,
                         EntradaActual, CuentaPulsos);
...
esp_err_t search_init_partition(uint8_t *pnumber) {
    ...
    *pnumber = 0;
    ...
    for (uint8_t i = 1; i <= 3; i++) {
        ...
        esp_err_t err = nvs_open_from_partition(pname, "storage", NVS_READWRITE,
                                                &my_handle);
        if (err != ESP_OK)
            ESP_LOGE("NVS", "ERROR IN NVS_OPEN");
...
void task_pulse(void *arg) {
    ...
    int pinLevel;
    /*** Inicialización de variables y ejecución de métodos para buscar la
     * última escritura***/
    ...
    while (1) {
        ...
        if (pinLevel == 1 && counted == false) {
            ...
            if (indexPv == PPP_LIMIT && indexEntradaActual == EPP_LIMIT &&
                ...
                if (partition_number <= PARTITIONS_MAX) {
                    ...
                    if (asprintf(&aux, "app%d", partition_number - 1) < 0) {
                        free(aux);
                        ESP_LOGE("ROTAR_NVS",
                                 "Nombre de particion no fue creado");
                    }
                    ESP_LOGI("DEINIT", "%s", aux);
                    err = nvs_flash_deinit_partition(aux);
                    if (err != ESP_OK)
                        ESP_LOGE("CNP", "ERROR (%s) IN DEINIT",
                                 esp_err_to_name(err));
                    else
                        free(aux);

                    // Inicializando la nueva partición
                    if (asprintf(&partition_name, "app%u", partition_number) <
                        0) {
                        free(partition_name);
                        ESP_LOGE("ROTAR_NVS",
                                 "Nombre de particion no fue creado");
                    ...
                    esp_err_t err = nvs_open_from_partition(
                        partition_name, "storage", NVS_READWRITE, &my_handle);
                    if (err != ESP_OK)
                        ESP_LOGE("NVS", "ERROR IN NVS_OPEN");
                    ...
                    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND)
                        ...
                        err =
                            nvs_set_u8(my_handle, "pnumber", partition_number);
                        if (err != ESP_OK)
                            ESP_LOGE("NVS", "ERROR IN SET");
                        err = nvs_commit(my_handle);
                        if (err != ESP_OK)
            ...
            /***** Aumenta pulsos *****/
            if (partition_number <= PARTITIONS_MAX) {
                err = contar_pulsos_nvs(&partition_name, &partition_number,
                                        &indexPv, &indexEntradaActual,
                                        &cuentaPulsosPv);
